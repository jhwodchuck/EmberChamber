...
export function BootstrapAuthForm({ mode }: { mode: BootstrapAuthMode }) {
  ...
  function moveToDeviceStep(requireInviteToken: boolean) {
    ...
    if (!validateMagicLinkAccess(requireInviteToken)) {
      setFormMessage({
        ...
        body:
          mode === "join"
            ? "Invite-only onboarding still needs a valid email, an age-gated confirmation, and the beta invite token when required."
            : "Sign-in still needs a valid email, an age-gated confirmation, and the invite token only when this address has never been used for beta access.",
      });
      return;
    }
  ...
  const subtitle =
    entryMethod === "device-link"
      ...
      : mode === "join"
        ? "First confirm the invite path and age-gated gate, then name the device that should receive the inbox link."
        : "First confirm the email and age-gated gate, then name the browser that should receive the inbox link.";
                  ...
                  <div className="rounded-[1.2rem] border border-[var(--border)] bg-[var(--bg-secondary)] px-4 py-4">
                    <label htmlFor={`${mode}-age-confirmed`} className="flex items-start gap-3">
                      ...
                      <span>
                        ...
                        >
                          EmberChamber beta access is age-gated. This is a self-attested gate, not identity verification.
                        </span>
                      </span>
                    </label>
                ...
                <>
                  <div className="rounded-[1.2rem] border border-[var(--border)] bg-[var(--bg-secondary)] px-4 py-4">
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-3">
                      ...
                      <div className="rounded-[1rem] border border-[var(--border)] bg-[var(--bg-primary)] px-3 py-3">
                        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-brand-600">Age-gated</p>

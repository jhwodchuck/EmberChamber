...
export function BootstrapAuthForm({ mode }: { mode: BootstrapAuthMode }) {
  ...
  const subtitle =
    mode === "join"
      ? "Age-gated access stays invite-gated. Your invite token is only needed for a new beta account."
      : "Returning users only need their private email, an age-gated affirmation, and a readable device name.";
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          <div className="rounded-[1.2rem] border border-[var(--border)] bg-[var(--bg-secondary)] px-3 py-3">
            ...
            <p className="mt-1 text-sm font-medium text-[var(--text-primary)]">
              {requiresInviteToken ? "Invite + email" : "Email"}
            </p>
          </div>
          <div className="rounded-[1.2rem] border border-[var(--border)] bg-[var(--bg-secondary)] px-3 py-3">

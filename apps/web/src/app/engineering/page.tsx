...
const evidence = [
  ...
  { title: "Checks and screenshots", description: "Inspect workflow results and their artifacts, not a hard-coded success badge.", href: `${githubRepoUrl}/actions` },
    ...
    <MarketingShell>
      <section className="px-6 pb-12 pt-14 sm:pt-20">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3"><a href={githubRepoUrl} className="btn-primary"><Github className="mr-2 h-4 w-4" aria-hidden="true" />Inspect the repository</a><Link href="/tour" className="btn-ghost">Take the product tour</Link></div>
          <p className="mt-5 max-w-3xl text-sm leading-7 text-[#cbb0a3]">Development is AI-assisted. Source history, repository guidance and workflow results provide the review trail; no user-count, uptime or independent-audit claims are made here.</p>
        </div>
      </section>
      ...
      <section className="px-6 py-10">
        <div className="mx-auto max-w-6xl">
          <h2 className="font-display text-4xl font-semibold text-[#fff1e8]">Inspect the evidence.</h2>
          <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{evidence.map((item) => <a key={item.title} href={item.href} className="rounded-2xl border border-white/10 bg-white/[0.025] p-5 transition-colors hover:border-[#ffb890]/40 hover:bg-white/[0.05]"><h3 className="font-semibold text-[#fff1e8]">{item.title}</h3><p className="mt-3 text-sm leading-7 text-[#d0b8ab]">{item.description}</p><span className="mt-4 inline-flex items-center gap-2 text-sm text-[#ffb890]">View on GitHub<ArrowRight className="h-4 w-4" aria-hidden="true" /></span></a>)}</div>
        </div>
      </section>
      ...
      <section className="px-6 pb-10 pt-8">
        <div className="mx-auto max-w-6xl rounded-3xl border border-white/10 bg-white/[0.03] p-7 sm:p-10">
          <p className="section-kicker">Work still in progress</p><h2 className="mt-4 font-display text-4xl font-semibold text-[#fff1e8]">A beta with visible boundaries.</h2>
          <p className="mt-5 max-w-3xl leading-8 text-[#d0b8ab]">Native attachment support, authenticator validation, recovery maturity and cross-client parity require continued verification. Communities and rooms have relay-hosted behavior that should not be confused with encrypted direct messages or new encrypted groups.</p>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-[#cbb0a3]">The README also documents historical release-version mismatches. Inspect the artifact and its notes rather than assuming a newer source commit means a newer installed client.</p>
          <div className="mt-6 flex flex-wrap gap-3"><Link href="/trust-and-safety" className="btn-ghost">Read the privacy boundaries</Link><a href={`${githubRepoUrl}#current-release-identity`} className="btn-ghost">Check release identity</a><Link href="/tour" className="btn-primary">See the product</Link></div>
        </div>
      </section>
    </MarketingShell>
  );

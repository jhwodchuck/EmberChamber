...
| Safety tooling                                  | Improved    | Disclosure-based reports are stored and reviewed in an operator console (queue, force-signout, recovery handoff) with a permanent audit log. | Add account suspension and bulk-review tooling; broaden audited intervention coverage. |
...
| Spam and raid attempts         | Invite-only access, auth/send rate limiting, small-group caps, block rows, operator report queue, community invite-freeze. | No bulk review queue or account suspension yet.                                                                |

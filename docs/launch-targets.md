...
| Windows         | NSIS `.exe` for beta/RC tags; `.exe` and `.msi` for stable tags from `apps/desktop`          | Relay        | Tauri shell bundling a static export of `apps/web`'s full authenticated workspace (auth, DMs, groups, communities/rooms, invites, sessions, privacy, search, settings, attachments); session/keys in the OS keyring. | `.github/workflows/release-windows.yml`                                            |
| Ubuntu / Debian | `.deb` and `.AppImage` from `apps/desktop`                                                   | Relay        | Same bundled `apps/web` static export as Windows.                                                                                                                                                                    | `.github/workflows/release-linux.yml`                                              |
| macOS           | `.app` and `.dmg` from `apps/desktop`                                                        | Relay        | Same bundled `apps/web` static export as Windows/Linux, but signed distribution and launch commitment remain later work.                                                                                             | `.github/workflows/release-macos.yml`                                              |
...
- Age-gated invite-gated onboarding is part of the current product contract across every committed surface.

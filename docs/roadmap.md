...
EmberChamber will be planned as a discreet, age-gated, invite-gated private messenger for trusted
circles. The initial cohort is privacy-conscious groups sharing protected media. Public product
framing remains privacy-first and broadly applicable. The roadmap will first
...
- Product boundary: age-gated is a permanent product rule.
- Public positioning: privacy-first encrypted messaging for trusted circles, described in clear and broadly applicable language.
- Initial users: privacy-conscious groups using the app to share sensitive media.
- Long-term market: broader age-gated trusted circles with varied privacy needs.
...
The repo is well ahead of where the original phase prose implied. A code-grounded audit:

- **Phase 0 (contract alignment): done.** Public copy, docs, and `AGENTS.md` reflect the
  age-gated, invite-gated, Android/Windows/Ubuntu/web positioning.
- **Phase 1 (closed-beta foundation): implemented.** 18+ affirmation, invite bootstrap,
  pseudonymous profiles, DMs, small groups, organizer-only invites, media, session
  review/revoke/recovery, joined-space search, standard/sensitive media defaults, and
  disclosure-based reporting are all present across relay + clients. Self-serve invite check
...
- **Phase 2 (closed-beta expansion): complete.**
  ...
  - _Also done (2026-06-16):_ **Mobile parity** — `CommunityScreen` added to the Expo Android client
    ...
    `communitySummary`, `communityDetail`, `conversationInviteDescriptor`,
...
- **Phase 3 (stable beta): in progress.**
  - _Done (2026-06-17):_ **Operational reporting & revocation tooling** — operator role (`is_operator`),
    ...
    magic link on the same account identity (the supported "lost all devices" path); a shared-secret
    `grant-operator` endpoint bootstraps the first operator. **Mobile test harness** — `jest-expo` unit
    suite (`apps/mobile`, wired into `verify:all` + CI). **Negative-path coverage** — relay error-path
...
- Success criterion:
  - a small invited private group can join, exchange private media, and keep using the product without major confusion or trust failures
...
- Success criterion:
  - a 100+ invite-gated age-gated community can operate with rooms, referrals, and media sharing without turning into a public social product

### Phase 3: Stable Beta

- Harden the age-gated trusted-circles product after community mechanics work.
- Focus on:
...
- Exit criterion:
  - the product is stable enough to serve a broader range of age-gated trusted circles
...
- Auth and bootstrap types:
  - add explicit `ageConfirmed18` or equivalent age-gated affirmation to onboarding
...
- Relay:
  ...
  - add age-gated affirmation handling and later community, room, and referral primitives
- Mobile and desktop:
...
- A new invited user with an invite completes onboarding, affirms 18+, chooses a pseudonymous identity, and lands in a usable DM or group flow.
...
- Age-gated uses self-attested 18+ gating, not heavy identity verification.
- Public copy stays discreet and broadly applicable to privacy-conscious groups.
